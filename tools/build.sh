...
# source, then links this repo's own objects (plus any --extra-obj-dir
# objects) into a flat ELF via `ld -T link.ld`.
#
...
#
# Requires paideia-as >= 0.36.0 (v1.1-A/L: real semantic-pipe emit +
# CNAME response-walker exercise the mov_b + xor+cmp + rip-relative
# lea patterns that shipped stable in 0.36).
#
# Usage:
...
if [ "$FAIL" -eq 0 ] && [ "${#OWN_OBJECTS[@]}" -gt 0 ]; then
    echo "[link] ld -T link.ld -> $BUILD_DIR/pdxdig.elf"
    ld -nostdlib --warn-common --fatal-warnings --gc-sections \
        -T link.ld \
        -o "$BUILD_DIR/pdxdig.elf" \
        "${OWN_OBJECTS[@]}" "${EXTRA_OBJECTS[@]}" "${EXTRA_ARCHIVES[@]}"
    echo "[link] OK -> $BUILD_DIR/pdxdig.elf"

    objcopy -O binary "$BUILD_DIR/pdxdig.elf" "$BUILD_DIR/pdxdig.bin"
    echo "[link] OK -> $BUILD_DIR/pdxdig.bin"
fi
